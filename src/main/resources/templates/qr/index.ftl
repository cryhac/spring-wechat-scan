...
<head>
    ...
    <title>Spring boot Wechat 扫描登录</title>
...
<script>
    const socket = new WebSocket("${projectUrlConfig.websocket}");
    ...
    socket.addEventListener('message', function (event) {
        ...
        console.log(data);
        if (data.messageType == 'qrcode_url') {
            $('#qrcode').attr('src', data.url);
        }
        if (data.messageType == 'scan_success') {
